...
| 리다이렉트 최대 | 3회 |
| 관용 경로 시도 최대 | 5개 (위 목록) |
| HTML alternate Feed 후보 최대 | 5개 |
| 탐색 1건의 총 HTTP 요청 최대 | 10회 (리다이렉트 포함) |
...
- HTML의 alternate Feed 링크와 리다이렉트는 같은 총 요청 예산을 사용한다. 예산을 넘으면 중단한다.
...
- IPv6 `2001::/23`은 IANA special-purpose 대역이므로 전체를 거부한다. 공개 Blog 탐색에는 전역 unicast 주소만 사용한다.
